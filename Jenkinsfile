        ...
        stage('Sonarqube') {
            steps {
                withSonarQubeEnv('sonarqube') {
                    sh "dotnet test /var/jenkins_home/workspace/peline-jenkins-dotnetcore_master/TDDTestProj/TDDTestProj.csproj /p:CollectCoverage=true /p:CoverletOutputFormat=opencover"
                    sh "dotnet build-server shutdown"
                    sh "dotnet sonarscanner begin /k:\"NetCoreTDD\" /d:sonar.host.url=\"http://10.0.75.1:9999\" /d:sonar.login=\"e698064a9d0e03485cb1eb35d8d7dc9aafaed425\"  /d:sonar.cs.opencover.reportsPaths=\"/var/jenkins_home/workspace/peline-jenkins-dotnetcore_master/TDDTestProj/coverage.opencover.xml\""
                    sh "dotnet  build  /var/jenkins_home/workspace/peline-jenkins-dotnetcore_master/TDD/TDD.sln"
